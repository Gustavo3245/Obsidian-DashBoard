...
- o primeiro resumo exibe `Total words` diretamente de `volume.snapshot.totalWords`, com formatação numérica e escala tipográfica baseada no tamanho do card.
...
- comandos, settings tab e ícone estão registrados; `Total words`, `Estimated time`, `Daily average words`, `File types`, `Tag insights`, `Recent activity` e `Writing streak` já possuem apresentação no layout-base;
