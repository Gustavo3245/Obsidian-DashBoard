...
- a primeira fileira começa com `Words` de `volume.snapshot.totalWords` e `Folders` de `volume.totalFolders`; a segunda exibe `Characters` de `volume.snapshot.totalCharacters` e `Files` de `volume.totalFiles`. Todos reutilizam o mesmo renderizador, formatação numérica e escala tipográfica baseada no tamanho do card; `Words` acrescenta a média persistida de palavras por arquivo, `Characters` acrescenta uma razão transitória entre caracteres e palavras, enquanto `Folders` e `Files` exibem a razão derivada de arquivos por pasta.
...
- comandos, settings tab e ícone estão registrados; `Words`, `Folders`, `Characters`, `Files`, `Estimated time`, `Daily average words`, `File types`, `Tag insights`, `Recent activity` e `Writing streak` já possuem apresentação no layout-base;
