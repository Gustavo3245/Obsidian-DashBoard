...
- `Recent activity` começa com uma lista vertical e possui um botão inferior apenas visual para consultar toda a atividade; entre 320px e 469px, enquanto a fileira ainda tiver dois cards, distribui os dez registros em duas colunas de cinco. Quando altura e largura permitem revelar o terceiro card, retorna a uma lista vertical.
- `Tag insights` lê do grupo persistido `appears` as tags mais usadas no Vault e no frontmatter e a tag menos usada, apresentando-as em uma lista; o total de tags únicas ocupa um bloco inferior separado, com divisor e valor destacado. Esse card existe apenas nos estados laterais de três colunas e no workspace central.
